...
if uploaded_file:
    try:
        ...
        if raw_df.empty:
            ...
            with st.container(border=True):
                st.markdown('<div class="section-label">📄 原始成绩表</div>', unsafe_allow_html=True)
        ...
        if raw_df.empty:
            ...
            with st.container(border=True):
                st.markdown('<div class="section-label">✅ 选择分析列</div>', unsafe_allow_html=True)
                st.markdown(
                    '<p class="section-note">系统会自动识别常见列名，也可以在这里手动确认。</p>',
                    unsafe_allow_html=True,
                )
                col_name, col_score = st.columns(2)
                name_col = col_name.selectbox(
                    "姓名列",
                    columns,
                    index=name_default_index,
                )
                score_col = col_score.selectbox(
                    "成绩列",
                    columns,
            ...
            with st.container(border=True):
                ...
                st.markdown(
                    '<p class="section-note">默认规则：优秀≥满分的90%，良好≥80%，及格≥60%。当前只开放优秀线调整。</p>',
                    unsafe_allow_html=True,
                )
            ...
            if name_col == score_col:
                st.error("请选择不同的姓名列和成绩列。")
                st.stop()

            selected = df[[name_col, score_col]].copy()
            selected.columns = ["姓名", "分数"]
            selected["姓名"] = selected["姓名"].astype(str).str.replace("\u3000", "", regex=False).str.strip()
            selected["分数"] = pd.to_numeric(selected["分数"], errors="coerce")
            ...
            if valid_scores.empty:
                st.error("没有可分析的有效成绩，请检查姓名列和成绩列。")
            else:
                ...
                with st.container(border=True):
                    ...
                    c5.metric("优秀人数", analysis_result["excellent_count"])
                ...
                with right:
                    with st.container(border=True):
                        st.markdown('<div class="section-label">🧭 不及格名单</div>', unsafe_allow_html=True)
                        if fail_df.empty:
                            st.info("本次没有不及格学生。")
                        else:
